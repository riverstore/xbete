...
string Cserver::insert_peer(const Ctracker_input& v, bool listen_check, bool udp, t_user* user)
{
	...
	t_peers::iterator i = file.peers.find(make_pair(v.m_ipa, v.m_port));
	if (i != file.peers.end())
	{
	...
	else if (user && user->torrents_limit && user->completes + user->incompletes >= user->torrents_limit)
		...
		if (i != file.peers.end())
		{
				...
				&& v.m_uploaded >= i->second.uploaded)
			{
				...
				if ((downloaded || uploaded) && timespent)
				{
					...
					if (m_config.m_cheater && upspeed > 900 << 10)
					{
						try
						{
      	...
      	if (m_config.m_seedbonus_on) {
 			
      		if (v.m_left==0 && timespent>=m_config.m_announce_interval) {
      			try {
      				Csql_query q(m_database, "UPDATE " + table_name(table_users) + " SET seedbonus=(`seedbonus`+ 0.2 ) where " + column_name(column_users_uid) + "= ? ");
      				     q.p(user->uid);
      				     q.execute();
      			}
      			catch (Cxcc_error) {}
	...
	if (v.m_event == Ctracker_input::e_stopped)
		file.peers.erase(make_pair(v.m_ipa, v.m_port));
	else
	{
		t_peer& peer = file.peers[make_pair(v.m_ipa, v.m_port)];
		...
		if (!m_config.m_listen_check || !listen_check)
			peer.listening = true;
		else if (!peer.listening && time() - peer.mtime > 1860)
		{
			Cpeer_link peer_link(v.m_ipa, v.m_port, this, v.m_info_hash, make_pair(v.m_ipa, v.m_port));
	...
	if (v.m_event == Ctracker_input::e_completed)
	{
		...
		if (m_config.m_snatched)
		{
			try
			{
		Csql_query q(m_database, "insert into xbt_snatched (uid, fid, downloaded, uploaded, tstamp) values (?, ?, ?, ?, unix_timestamp())");
				q.p(user->uid);
				q.p(file.fid);
			    q.p(v.m_downloaded);
			    q.p(v.m_uploaded);
...
void Cserver::update_peer(const string& file_id, pair<int, int> peer_id, bool listening)
{
...
void Cserver::read_db_users()
{
	...
	try
	{
		Csql_query q(m_database, "select ?, name, pass, torrent_pass, wait_time, torrents_limit, peers_limit, torrent_pass_secret, can_leech, seedbonus from ?");
...
int Cserver::test_sql()
{
	...
	try
	{
		...
		m_database.query("select " + column_name(column_users_uid) + ", name, pass, can_leech, wait_time, peers_limit, torrents_limit, torrent_pass, downloaded, uploaded, torrent_pass_secret, seedbonus from " + table_name(table_users) + " where 0 = 1");
		return 0;
	}
	catch (Cxcc_error)
	{
